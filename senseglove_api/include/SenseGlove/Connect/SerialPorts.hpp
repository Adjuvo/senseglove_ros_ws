...
public:
    ...
    [[nodiscard]] static bool IsBluetoothPort(const serial::PortInfo& portInfo);

    /// <summary> Returns true if this Bluetooth port can be opened without locking the application. </summary>
    [[nodiscard]] static bool IsValidBluetoothPort(const serial::PortInfo& portInfo);

    /// <summary> Returns true if this hardware id belongs to a Sense Glove Device. </summary>
    [[nodiscard]] static bool IsSenseGlovePort(const serial::PortInfo& portInfo);

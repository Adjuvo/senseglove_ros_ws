...
public:
    ...
    [[nodiscard]] int32_t GetFirmwareVersion() const;

    /// <summary> Sub-Firmware version running on the device's MicroController. </summary>
    [[nodiscard]] int32_t GetSubFirmwareVersion() const;
...
public:
    ...
    [[nodiscard]] bool Equals(const SenseGloveInfo& senseGloveGloveInfo, bool bGeometryOnly = false) const;

public:
    /// <summary> Create a string representation for logging. If shortNotation is false, all values are reported.
    /// </summary>
    [[nodiscard]] std::string ToString(bool bShortNotation) const;

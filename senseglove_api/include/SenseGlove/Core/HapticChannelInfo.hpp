...
public:
    ...
    /// <returns></returns>
    [[nodiscard]] static std::string GenerateAddress(int32_t deviceIndex, int32_t channelIndex);

    /// <summary> Parse HapticChannelInfo received through IPC into usable data. </summary>
    /// <param name="ipcString"></param>
    /// <returns></returns>
    [[nodiscard]] static HapticChannelInfo Parse(const std::string& ipcString);

public:
    /// <summary> Byte used to identify streaming channels. </summary>
    [[nodiscard]] static int32_t GetStreamingType();

    /// <summary> Byte used to indicate fire-and-forget channels. </summary>
    [[nodiscard]] static int32_t GetFireAndForgetType();
...
public:
    ...
    /// <returns></returns>
    [[nodiscard]] int32_t GetStreamChannelCount() const;
    ...
    /// <returns></returns>
    [[nodiscard]] int32_t GetFireAndForgetChannelCount() const;

    /// <summary> Retrieve the total amount of channels for this device. </summary>
    /// <returns></returns>
    [[nodiscard]] int32_t GetTotalChannelCount() const;
    ...
    /// <returns></returns>
    [[nodiscard]] std::string GetAddress_Unguarded(EHapticChannelType type, int32_t typeIndex) const;
    ...
    /// <returns></returns>
    [[nodiscard]] std::vector<std::string> GetAllAddresses() const;
};
...
public:
    ...
    [[nodiscard]] bool Equals(const HandPose& handPose) const;
    ...
    [[nodiscard]] float GetNormalizedFlexion(EFinger finger, bool bClamp01 = true) const;

    /// <summary> Returns the total flexion the fingers as a value between 0 (fully extended) and 1 (fully flexed).
    /// </summary>
    /// <remarks> Useful for animation or for detecting gestures. </remarks>
    [[nodiscard]] std::vector<float> GetNormalizedFlexion(bool bClamp01 = true) const;
...
protected:
    ...
    /// exceptions. </remarks>
    [[nodiscard]] float GetNormalizedFlexion(int32_t finger, bool bClamp01 = true) const;

public:
    [[nodiscard]] std::string ToString(bool bShortFormat = false) const;

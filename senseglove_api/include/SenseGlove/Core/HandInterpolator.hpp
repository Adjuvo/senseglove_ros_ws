...
public:
    ...
    [[nodiscard]] float CalculateAngle(EFingerMovement movement, float value, bool bApplyLimits = true) const;

    /// <summary> Calculate a thumb movement's rotation, using an input value. </summary>
    [[nodiscard]] float CalculateAngle(EThumbMovement movement, float value, bool bApplyLimits = true) const;
...
public:
    ...
    /// <returns></returns>
    [[nodiscard]] bool Equals(const HandInterpolator& handInterpolator) const;

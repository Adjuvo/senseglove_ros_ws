...
public:
    ...
    [[nodiscard]] static char GetThumperByte();
...
public:
    ...
    [[nodiscard]] const SenseGloveInfo& GetGloveInfo() const;

    /// <summary> Offset from thimble to fingertip, used for Inverse Kinematics. </summary>
    [[nodiscard]] const std::vector<Kinematics::Vect3D>& GetFingerThimbleOffsets() const;

    void SetFingerThimbleOffset(int32_t finger, const Kinematics::Vect3D& offset);

    [[nodiscard]] bool SupportsWristActuator() const;
...
protected:
    ...
    /// <returns></returns>
    [[nodiscard]] std::string ToSenseGloveCommand(

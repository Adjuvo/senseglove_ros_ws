...
public:
    ...
    [[nodiscard]] static BasicHandModel Default(bool bRightHanded);

    /// <summary> Convert a serialized HandModel back into its class representation. </summary>
    [[nodiscard]] static BasicHandModel Deserialize(const std::string& serializedString);
...
public:
    ...
    [[nodiscard]] static const std::vector<std::vector<float>>& GetBaseFingerLengths();
    ...
    [[nodiscard]] static const std::vector<Vect3D>& GetBaseJointPositions();
    ...
    [[nodiscard]] static const std::vector<Vect3D>& GetBaseJointAngles();
...
public:
    ...
    [[nodiscard]] const std::vector<std::vector<float>>& GetFingerLengths() const;

    /// <summary> Retrieve the finger lengths of a specific finger in mm. </summary>
    [[nodiscard]] const std::vector<float>& GetFingerLengths(EFinger finger) const;
    ...
    [[nodiscard]] const std::vector<std::vector<float>>& GetFingerRatios() const;
...
public:
    ...
    [[nodiscard]] const std::vector<float>& GetFingerRatios(EFinger finger) const;

    /// <summary> Starting joint positions relative to the device Origin. </summary>
    [[nodiscard]] const std::vector<Vect3D>& GetStartJointPositions() const;
    ...
    [[nodiscard]] const std::vector<Quat>& GetStartJointRotations() const;

    /// <summary> Retrieve the start position of a specific finger. </summary>
    [[nodiscard]] const Vect3D& GetJointPosition(EFinger finger) const;
    ...
    [[nodiscard]] const Quat& GetJointRotation(EFinger finger) const;
    ...
    [[nodiscard]] const std::vector<float>& GetTotalLengths() const;

    /// <summary> Get the total length of a specific finger, in mm. </summary>
    [[nodiscard]] float GetTotalLength(EFinger finger) const;
...
public:
    ...
    [[nodiscard]] std::vector<Vect3D> Get3DLengths(EFinger finger) const;
...
public:
    ...
    /// <returns></returns>
    [[nodiscard]] bool Equals(const BasicHandModel& basicHandModel) const;
...
public:
    ...
    /// <returns></returns>
    [[nodiscard]] std::string ToString(bool bLengthsOnly) const;

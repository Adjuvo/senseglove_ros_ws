...
public:
    ...
    [[nodiscard]] float Get(float value, bool bLimit, bool bNormalizeAngle = false) const;

public:
    /// <summary> Returns true if this InterpolationSet has the same values as another set. </summary>
    /// <param name="other"></param>
    /// <returns></returns>
    [[nodiscard]] bool Equals(const InterpolationSet& interpolationSet) const;

public:
    [[nodiscard]] std::string ToString() const;

    [[nodiscard]] std::string ToString(bool bLimits) const;

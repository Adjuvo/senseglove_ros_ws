...
public:
    ...
    [[nodiscard]] static bool Deserialize(const std::string& serialized, SensorNormalization& out_normalizer);
...
public:
    ...
    /// values. </summary>
    [[nodiscard]] bool CollectsNormalization() const;
...
public:
    ...
    [[nodiscard]] int32_t GetMoveCount() const;

    /// <summary> Returns true if all sensors are allowed to move. </summary>
    [[nodiscard]] bool AllSensorsMoving() const;
    ...
    /// <param name="index"></param>
    /// <returns></returns>
    [[nodiscard]] bool MovedEnough(int32_t index) const;
    ...
    /// <returns></returns>
    [[nodiscard]] float Denormalize(float value01, int32_t index, float fallbackValue = 1.0f) const;

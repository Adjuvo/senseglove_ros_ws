...
public:
    ...
    [[nodiscard]] Vect3D ToEuler() const;

    /// <summary> Rotate Vect3D by this quaternion rotation. </summary>
    [[nodiscard]] Vect3D Rotate(const Vect3D& vect3D) const;

    /// <summary> Check if two quaternions have equal values. </summary>
    [[nodiscard]] bool Equals(const Quat& quat) const;
    ...
    /// <returns></returns>
    [[nodiscard]] static Quat LookRotation_XYZ(const Vect3D X, const Vect3D Y,
...
public:
    [[nodiscard]] std::string Serialize(const char delimiter = ',') const;
};
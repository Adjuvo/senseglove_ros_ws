...
controller_interface::CallbackReturn SenseGloveStateBroadcaster::on_init()
{
  ...
  pub_ = node->create_publisher<senseglove_msgs::msg::SenseGloveState>(topic_name_, rclcpp::QoS(10).best_effort());
...
static inline double read_value(
    hardware_interface::LoanedStateInterface &sg_interface,
    ...
    const char *logger_name)
{
  ...
  RCLCPP_WARN_THROTTLE(logger, *clock, 2000, "State interface '%s' has no value; using 0.0", logger_name);
  return 0.0;
}

// Update
controller_interface::return_type
SenseGloveStateBroadcaster::update(const rclcpp::Time &time, const rclcpp::Duration &)
{
  if (!pub_) return controller_interface::return_type::OK;
  ...
  if (state_interfaces_.size() < expected) {
    RCLCPP_WARN_THROTTLE(get_node()->get_logger(), *get_node()->get_clock(), 2000,
        "State interfaces provided (%zu) < expected (%zu). Skipping publish.",state_interfaces_.size(), expected);
  ...
  for (size_t i = 0; i < joint_size; ++i) {
    msg.position[i]          = read_value(state_interfaces_[index++], logger, clock,
                                                  (joint_names_[i] + "/position").c_str());
    msg.absolute_velocity[i] = read_value(state_interfaces_[index++], logger, clock,
                                                  (joint_names_[i] + "/velocity").c_str());
  }
  ...
  for (int i = 0; i < 20; ++i) {
    ...
    hp.x = read_value(state_interfaces_[index++], logger, clock,
                              ("hand_joint_" + std::to_string(i) + "/position.x").c_str());
    hp.y = read_value(state_interfaces_[index++], logger, clock,
                              ("hand_joint_" + std::to_string(i) + "/position.y").c_str());
    hp.z = read_value(state_interfaces_[index++], logger, clock,
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    p.x = read_value(state_interfaces_[index++], logger, clock,
                             ("finger_tip_" + std::to_string(i) + "/position.x").c_str());
    p.y = read_value(state_interfaces_[index++], logger, clock,
  ...
  // IMU quaternion
  msg.imu_orientation.x = read_value(state_interfaces_[index++], logger, clock, "imu/orientation.x");
  msg.imu_orientation.y = read_value(state_interfaces_[index++], logger, clock, "imu/orientation.y");
  msg.imu_orientation.z = read_value(state_interfaces_[index++], logger, clock, "imu/orientation.z");
  msg.imu_orientation.w = read_value(state_interfaces_[index++], logger, clock, "imu/orientation.w");
...
PLUGINLIB_EXPORT_CLASS(senseglove_state_broadcaster::SenseGloveStateBroadcaster, controller_interface::ControllerInterface)